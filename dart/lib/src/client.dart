...
class AdviceValidationException implements Exception {
  final List<String> errors;

  AdviceValidationException(this.errors);

  @override
  String toString() => 'AdviceValidationException: ${errors.join("; ")}';
}
...
/// Advice Generator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/advice?utm_source=dart&utm_medium=readme
///
/// Parameters:
...
class AdviceClient {
  ...
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw AdviceValidationException(errors);
    }
  }

  /// Execute a request to the Advice Generator API.
  ///
  /// Parameters are validated before sending the request.
  Future<AdviceResponse> execute() async {
    if (apiKey.isEmpty) {
      throw AdviceException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw AdviceException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw AdviceException('Resource not found');
      } else {
        throw AdviceException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is AdviceException) rethrow;
      throw AdviceException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Advice Generator API client.
class AdviceException implements Exception {
  final String message;

  AdviceException(this.message);

  @override
  String toString() => 'AdviceException: $message';
}
